...
class ValPre(object):
    ...
    def __call__(self, rgb, gt, modal_x):
        ...
        target_size = getattr(self.config, "force_val_size", None)
        if target_size is not None:
            tgt_h, tgt_w = target_size
            rgb = cv2.resize(rgb, (tgt_w, tgt_h), interpolation=cv2.INTER_LINEAR)
            gt = cv2.resize(gt, (tgt_w, tgt_h), interpolation=cv2.INTER_NEAREST)
            modal_x = cv2.resize(modal_x, (tgt_w, tgt_h), interpolation=cv2.INTER_LINEAR)
